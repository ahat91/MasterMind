...
+ (NSIndexSet *) convertArrayToNSIndexSet:(int *)array
{
    int arrayCount = sizeof(array) / sizeof(array[0]);
...
+ (NSIndexPath *) convertJsonToIndexPath:(int *)array
{
    int arrayCount = sizeof(array) / sizeof(array[0]);
    NSUInteger indexes[arrayCount];
    for(NSInteger i = 0; i < arrayCount; i++)
        indexes[i] = (NSUInteger)[NSNumber numberWithInt:array[i]];
...
+ (NSIndexPath *) convertJsonToIndexPath_stringarray:(NSArray *)array
{
    ...
    for(NSInteger i = 0; i < array.count; i++)
        indexes[i] = (NSUInteger)[NSNumber numberWithInt:[array[i] intValue]];
