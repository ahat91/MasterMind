...
public static class MHTools {
	...
	public static DateTime ConvertJSONToDateTime(string jsonString)
	{
		...
		if(dateDecoded != null)
		{
			if((string)dateDecoded["ObjectType"] == "Date")
				return new DateTime((Convert.ToInt32(dateDecoded["Year"])), (Convert.ToInt32(dateDecoded["Month"])), (Convert.ToInt32(dateDecoded["Day"])), (Convert.ToInt32(dateDecoded["Hour"])), (Convert.ToInt32(dateDecoded["Minute"])), (Convert.ToInt32(dateDecoded["Second"])));
		}
	...
	public static string ConvertStringArrayToJson(string[] array)
	{
		return (new ArrayList(array).toJson());
	}
	
	public static string ConvertIntArrayToJson(int[] array)
	{
		return (new ArrayList(array).toJson());
	}
	
	public static string ConvertIntDoubleArrayToJson(int[][] array)
	{
		ArrayList resultList = new ArrayList();
		for(int i = 0; i < array.Length; i++)
			resultList.Add(new ArrayList(array[i]).toJson());
		return (resultList.toJson());
	}
	
	public static MHTouch ConvertJsonToTouch(string jsonString)
	{
		Hashtable touchDecoded = jsonString.hashtableFromJson();
		if(touchDecoded != null)
		{
			if((string)touchDecoded["ObjectType"] == "Touch")
			{
				Vector2 locationInView = ConvertJSONToVector2((string)touchDecoded["locationInView"]);
				Vector2 previousLocationInView = ConvertJSONToVector2((string)touchDecoded["previousLocationInView"]);
				MHView view = MHiOSManager.GetObjectByUniqueTag(Convert.ToInt32(touchDecoded["view"])) as MHView;
				int tapCount = Convert.ToInt32(touchDecoded["tapCount"]);
				float timeStamp = (float)((double)touchDecoded["timeStamp"]);
				MHTouchPhase phase = (MHTouchPhase)Convert.ToInt32(touchDecoded["phase"]);
				/*
				MHGestureRecognizer[] gestureRecognizers = null;
				ArrayList intArrayDecoded = ((string)touchDecoded["gestureRecognizers"]).arrayListFromJson();
				if(intArrayDecoded != null)
				{
					int[] resultArray = new int[intArrayDecoded.Count];
					for(int i = 0; i < intArrayDecoded.Count; i++)
						resultArray[i] = Convert.ToInt32(intArrayDecoded[i]);
					gestureRecognizers = MHiOSManager.GetObjectsByUniqueTags(resultArray) as MHGestureRecognizer[];
				}
				*/
				return new MHTouch(locationInView, previousLocationInView, view, tapCount, timeStamp, phase);
			}
		}
		return new MHTouch();
	}
	
	public static MHTouch[] ConvertJsonToTouches(string jsonString)
	{
		MHTouch[] touches = null;
		ArrayList touchArrayDecoded = jsonString.arrayListFromJson();
		if(touchArrayDecoded != null)
		{
			touches = new MHTouch[touchArrayDecoded.Count];
			for(int i = 0; i < touchArrayDecoded.Count; i++)
				touches[i] = ConvertJsonToTouch(Convert.ToString(touchArrayDecoded[i]));
		}
		return touches;
	}
	
	public static MHEvent ConvertJsonToEvent(string jsonString)
	{
		Hashtable eventDecoded = jsonString.hashtableFromJson();
		if(eventDecoded != null)
		{
			if((string)eventDecoded["ObjectType"] == "Event")
			{
				MHTouch[] allTouches = ConvertJsonToTouches((string)eventDecoded["allTouches"]);
				float timeStamp = (float)((double)eventDecoded["timeStamp"]);
				MHEventType type = (MHEventType)Convert.ToInt32(eventDecoded["type"]);
				MHEventType subtype = (MHEventType)Convert.ToInt32(eventDecoded["subtype"]);
				
				return new MHEvent(allTouches, timeStamp, type, subtype);
			}
		}
		return new MHEvent();
	}
	...
	{
		string result = "";
		foreach(int key in MHiOSManager.Instance.objectsByTag.Keys)
			result += key + ": " + MHiOSManager.Instance.objectsByTag[key].GetType() + " " + MHiOSManager.Instance.objectsByTag[key].GetHashCode() + "\n";
		Debug.Log(result);
	}
