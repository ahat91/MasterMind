...
public partial class MHiOSManager : Singleton<MHiOSManager> {
	...
	[DllImport("__Internal")]
    private static extern void _addActionForControlEvents_2(int tag, int action, int controlEvents);
	public void addActionForControlEvents(int tag, Action<MHControl> action, MHControlEvents controlEvents)
	{
		if( Application.platform == RuntimePlatform.IPhonePlayer )
        {
			int actionTag = SaveActionToUniqueID(action);
			
			_addActionForControlEvents_2(tag, actionTag, (int)controlEvents);
		}
	}
	
	[DllImport("__Internal")]
    private static extern void _addActionForControlEvents_3(int tag, int action, int controlEvents);
	public void addActionForControlEvents(int tag, Action<MHControl, MHEvent> action, MHControlEvents controlEvents)
	{
		if( Application.platform == RuntimePlatform.IPhonePlayer )
        {
			int actionTag = SaveActionToUniqueID(action);
