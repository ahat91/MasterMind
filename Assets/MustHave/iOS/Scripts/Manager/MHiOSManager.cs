...
public partial class MHiOSManager : Singleton<MHiOSManager> {
	...
	public Dictionary<int, MHObject> objectsByTag = new Dictionary<int, MHObject>();
	...
	{
		if(Instance.objectsByTag.ContainsKey(tag))
	...
	public static MHObject GetObjectByUniqueTag(int tag)
	{
		if(tag != 0)
		{
			if(Instance.objectsByTag.ContainsKey(tag))
				...
				Debug.LogWarning("ERROR: This tag, " + tag + ", does not associate with any object.");
			return null;
		}
		...
		if(actionDecoded != null)
		{
			if(Convert.ToString(actionDecoded["ObjectType"]) == "Action")
			{
				...
				if(action != null)
				{
					...
					else if(action is Action<MHControl>)
					{
						int paramTag = Convert.ToInt32(actionDecoded["Param1"]);
						MHControl param1 = GetObjectByUniqueTag(paramTag) as MHControl;
						(action as Action<MHControl>).Invoke(param1);
					}
					else if(action is Action<MHControl, MHEvent>)
					{
						int paramTag = Convert.ToInt32(actionDecoded["Param1"]);
						MHControl param1 = GetObjectByUniqueTag(paramTag) as MHControl;
						string eventCoded = Convert.ToString(actionDecoded["Param2"]);
						MHEvent param2 = MHTools.ConvertJsonToEvent(eventCoded);
						(action as Action<MHControl, MHEvent>).Invoke(param1, param2);
					}
