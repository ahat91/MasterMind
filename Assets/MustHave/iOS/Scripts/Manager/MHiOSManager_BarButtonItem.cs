...
public partial class MHiOSManager : Singleton<MHiOSManager> {
	...
	[DllImport("__Internal")]
    private static extern int _style_barbuttonitem(int tag);
	public MHBarButtonItemStyle style_barbuttonitem(int tag)
	{
		if( Application.platform == RuntimePlatform.IPhonePlayer )
        {
			return (MHBarButtonItemStyle)_style_barbuttonitem(tag);
		}
		else 
	...
	[DllImport("__Internal")]
    private static extern void _style_barbuttonitem_set(int tag, int style);
	public void style_barbuttonitem(int tag, MHBarButtonItemStyle style)
	{
		if( Application.platform == RuntimePlatform.IPhonePlayer )
        {
			_style_barbuttonitem_set(tag, (int)style);
		}
	}
	...
	public void possibleTitles(int tag, string[] titles)
	{
		if( Application.platform == RuntimePlatform.IPhonePlayer )
        {
			_possibleTitles_set(tag, MHTools.ConvertStringArrayToJson(titles));
		}
	}
